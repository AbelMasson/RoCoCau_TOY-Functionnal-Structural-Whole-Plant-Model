...
class VoxNet(object):
    ...
    def split_db(self, val_size):
        ...
        self.train_size = 80000 - self.val_size
    ...
    def test_one_step(self, no_epoch):
        ...
        if no_epoch == 1 :
            np.save(os.path.join(self.path_to_metrics, 'y_ID_testFT_' + str(self.target_name[0:-4]) + '.npy'), self.IDs_test)

        y_pred = self.model.predict(X_test, verbose=1, steps=1)
        
        np.save(os.path.join(self.path_to_metrics, 'y_val_' + str(self.target_name[0:-4]) + '_predFT_epoch_{}.npy'.format(no_epoch)), y_pred)
        ...
        destination = ''.join([self.path_to_weights, 'model_devineFT_' + str(self.target_name[0:-4]) + '_epoch_{}.h5'.format(no_epoch)])
    ...
    def train_and_monitor(self, path_to_data, path_modelLH, path_model, path_to_metrics, path_to_weights, batch_size,
        ...
        self.no_epoch = 0
        self.path_modelLH = path_modelLH
        ...
        #Rajout du fine-tuning
        self.model = keras.models.load_model(self.path_modelLH)
        self.model.compile(loss=self.loss_function,
                       optimizer=self.optimizer,
                       metrics=self.metrics)
        self.model.save(self.path_model)
        ...
        np.save(os.path.join(self.path_to_metrics, 'val_lossFT_'+ str(self.target_name[0:-4]) + '_per_epoch.npy'), self.val_loss)
        np.save(os.path.join(self.path_to_metrics, 'train_lossFT' + str(self.target_name[0:-4]) + '.npy'), self.train_loss)
        np.save(os.path.join(self.path_to_metrics, 'train_metricsFT' + str(self.target_name[0:-4]) +'.npy'), self.train_metrics)
