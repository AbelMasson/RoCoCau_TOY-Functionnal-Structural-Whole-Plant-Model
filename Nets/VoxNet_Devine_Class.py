...
class VoxNet(object):
    ...
    def __init__(self, target_name, target_size):
        '''
        ...
        self.model.add(Dense(self.target_size))
    ...
    def fit_one_step(self):
        ...
        for i in range(self.n_batch_per_step):
            self.Batch = BatchGenerator(self.path_to_data, self.list_IDs[i], batch_size=self.batch_size, target_name=self.target_name, target_size=self.target_size)
        ...
        if self.iter % 10 == 0:
            self.model.save(''.join([self.path_to_weights, 'model_devine_' + str(self.target_name[0:-4]) + '_iter_{}.h5'.format(self.iter)]))
    ...
    def test_one_step(self, no_epoch):
        ...
        Batch = BatchGenerator(self.path_to_data, self.IDs_test, self.batch_size, target_name=self.target_name, target_size=self.target_size)
        X_test, y_test = Batch.get_Batch_raw_paramater()

        y_pred = self.model.predict(X_test, verbose=1, steps=1)
        
        np.save(os.path.join(self.path_to_metrics, 'y_val_' + str(self.target_name[0:-4]) + '_pred_epoch_{}.npy'.format(no_epoch)), y_pred)
        
        score = self.model.evaluate(X_test, y_test, verbose=0, steps=1)
        self.val_loss.append(score[0])
        print(f'Test loss: {score[0]} / Test mean_absolute_percentage_error: {score[1]}')
        ...
        destination = ''.join([self.path_to_weights, 'model_devine_' + str(self.target_name[0:-4]) + '_epoch_{}.h5'.format(no_epoch)])
    ...
    def train_and_monitor(self, path_to_data, path_model, path_to_metrics, path_to_weights, batch_size,
        ...
        #Rajout du fine-tuning sur l'entrainement de la taproot
        if self.target_name == 'targets_paramtaproot.npy' :
            
            self.model = keras.models.load_model('/home/abel/Desktop/VoxNet_Strat_Sauvegarde/Models/model_paramtrunk_DB_tot_StressLH.h5')
            self.model.compile(loss=self.loss_function,
                           optimizer=self.optimizer,
                           metrics=self.metrics)
            self.model.save(self.path_model)
            ...
            self.Batch_test = BatchGenerator(self.path_to_data, self.IDs_test, batch_size=self.batch_size, target_name=self.target_name, target_size=self.target_size)
            self.X_test, self.y_test = self.Batch_test.get_Batch_raw_paramater()
            y_test_arr = self.Batch_test.get_y_test()
            np.save(os.path.join(self.path_to_metrics, 'y_val_'+ str(self.target_name[0:-4]) +'.npy'), y_test_arr)
        ...
        np.save(os.path.join(self.path_to_metrics, 'val_loss_'+ str(self.target_name[0:-4]) + '_per_epoch.npy'), self.val_loss)
        np.save(os.path.join(self.path_to_metrics, 'train_loss' + str(self.target_name[0:-4]) + '.npy'), self.train_loss)
        np.save(os.path.join(self.path_to_metrics, 'train_metrics' + str(self.target_name[0:-4]) +'.npy'), self.train_metrics)
